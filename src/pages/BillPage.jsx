...
export default function BillPage() {
  ...
  const activeSecObj = useMemo(
    () => sections.find((s) => s.sectionId === activeSection),
    [sections, activeSection]
  )

  // ✅ Build section map only once
  const productsBySection = useMemo(() => {
    const map = {}

    for (const product of allProducts) {
      if (!map[product.sectionId]) {
        map[product.sectionId] = []
      }
      map[product.sectionId].push(product)
    }
  ...
  const displayProducts = useMemo(() => {
    ...
    if (activeSecObj?.subsections?.length) {
      activeSecObj.subsections.forEach((sub) => {
        const subProducts = productsBySection[sub.id] || []
  ...
  const grouped = useMemo(() => {
    ...
    if (activeSecObj.subsections?.length) {
      activeSecObj.subsections.forEach((sub) => {
        const prods = productsBySection[sub.id] || []
      ...
      groups.push({
        label: activeSecObj.label,
        id: activeSecObj.sectionId,
        products: displayProducts,
      })
    }
    ...
    <div className="flex h-[calc(100vh-57px)]">
      ...
      <div
        className={`flex flex-col ${
          showBill ? 'hidden' : 'flex'
        } md:flex flex-col w-full md:w-[55%] lg:w-[60%] overflow-hidden border-r border-ink-700/30`}
      >
        {/* Search + tabs */}
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder={
                listening
                  ? 'Listening…'
                  : 'Search products… (by Rate or by Name)'
              }
              ...
              style={{
                paddingRight: micSupported ? '4.5rem' : '2.5rem',
              }}
            />
      ...
      <div
        className={`${
          showBill ? 'flex' : 'hidden'
        } md:flex flex-col w-full md:w-[45%] lg:w-[40%]`}
      >
        <BillPanel />
      </div>
