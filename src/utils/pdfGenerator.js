...
export async function generateBillPDF(bill, shop = {}) {
  // 🔥 lazy load (performance improvement)
...
/* ------------------ UPDATED CALLERS (IMPORTANT) ------------------ */

export async function downloadBillPDF(bill, shop) {
  const doc = await generateBillPDF(bill, shop)
  doc.save(`Bill_${bill.billNo || 'draft'}.pdf`)
}

export async function previewBillPDF(bill, shop) {
  const doc = await generateBillPDF(bill, shop)
  const url = doc.output('bloburl')
  window.open(url, '_blank')
}

export async function getBillPDFBlob(bill, shop) {
  const doc = await generateBillPDF(bill, shop)
  return doc.output('blob')
}

export async function sharePDFWhatsApp(bill, shop, phone) {
  const blob = await getBillPDFBlob(bill, shop)
